...
export class AdminService {
  ...
  async getGameState(): Promise<GameState & { reachablePositions?: number[] }> {
    ...
    const reachablePositions = this.getReachablePositions(state.currentPosition, state.totalMoves);
    
    return { ...state, reachablePositions };
  }

  // Возвращает список позиций, которые будут посещены в течение всей игры
  private getReachablePositions(currentPosition: number, totalMoves: number): number[] {
    const DICE_SEQUENCE = [
      3, 1, 2, 2, 1, 5, 3, 2, 1, 4,
      2, 1, 6, 1, 2, 3, 5, 1, 2, 3,
      1, 6, 1, 5, 2, 1, 3, 1, 5, 2,
  ...
  async updateWelcomeBanner(message: string, enabled: boolean): Promise<WelcomeBanner> {
    let banner = await this.welcomeBannerRepository.findOne({ where: { id: 1 } });
